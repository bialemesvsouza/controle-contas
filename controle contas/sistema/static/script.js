...
async function loadDashboard() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            // NOVO: Cálculo e Exibição do Balanço
            ...
            if(elBalanco) {
                ...
                // Muda a cor: Verde se >= 0, Vermelho se < 0
...
// Antes era: if(!confirm(...)) return;
// Agora fica assim:

function excluirCategoria(id) {
    abrirConfirmacao("Deseja realmente excluir esta categoria?", async () => {
        // Todo o código que antes ficava depois do confirm agora fica aqui dentro
        try {
...
function renderizarTabela() {
    ...
    listaFiltrada.forEach((p) => {
        ...
        if(p.status === 'atrasado') badge = 'status-atrasado';

        const checkboxHtml = p.status !== 'pago' 
...
function confirmarExclusao() {
    // Note que aqui estamos chamando um modal por cima de outro modal
